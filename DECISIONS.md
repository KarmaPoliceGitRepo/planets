...
| I-6 | Mitigated | **Correctness (HIGH):** fixed — source-digest baseline now recorded (CR-H6), atomic autosave (CR-H7), Event-backed CancelToken (CR-H8), escaped drawtext/metadata (CR-H12). Deferred — A/V crossfade desync (CR-H10) and silence-parse mispair (CR-H11) need golden-output tests first (→ I-7). Disputed — CR-H9 ducking is correct on re-review (Won't-fix). | Schedule CR-H10/H11 with render golden tests. |
| I-7 | Open | **Quality/debt:** fixed CR-M7/M8/M10/M11 + CR-L1/L8. Deferred MED/LOW: subprocess return-code checks (CR-M2), master measurement (CR-M3/M4), caption/chapter timing (CR-M5/M6), seek accuracy (CR-M1), adjacency heuristic (CR-M9), fps rounding (CR-L2), shared silence parser (CR-L3/M12), and CR-L4..L11. | Batch-fix behind render golden tests; fold structural items into TD register. |
