...
| ADR-014 | 2026-06-24 | Accepted | **Podcast SoI model raised to formal-SysML parity with ReelCut.** Added `11-formal-structure.md` (every component C0–C10/C4b/C5b + the SoI as `«block»`s with named+described ports, operations, value properties; interface blocks I-Mic…I-Backup realizing IR-1…4; functions F1–F10 with typed I/O flows; value-type catalogue; component BDD + context IBD), `12-formal-behaviour.md` (use-case diagram, control-flow activity, state machine with modes M1–M5 as states + recovery, remote-interview sequence; behaviours enumerated nominal/alt/exception/edge), `13-parametrics-and-requirements.md` (MoE-1…6 as `«constraintBlock»`s bound to PR values + parametric diagram, requirements diagram, and orphan resolution: K1–K6 `«trace»`/`«evaluate»`, modes as states, formal CFG-* ids, N-28 gap). Result: **every podcast element now has named+described structural & behavioural features and ≥1 relationship; 18/18 podcast diagrams render.** | HARD rule from the user: all elements must have all structural/behavioural features defined+named+described and be linked via a relevant relationship; all SysML diagrams complete. Brings the SoI model (previously prose/RTM) to the same formal standard the ReelCut sub-model already met. |

| ADR-015 | 2026-06-24 | Accepted | **Deep SysML completeness — four-question pass + agent pipeline.** Drove both models to YES on four questions: (Q1) every use case has its own **activity-decomposition** diagram (ReelCut UC-1..10 in `2a-use-case-activities.md`, podcast UC-P1..5 in `14-use-case-activities.md`); (Q2) every **IBD has a matching interaction (sequence) diagram** exercising its ports (ReelCut `6-interaction-model.md §8` + completed Logical-Subsystems IBD connectors; podcast `12 §12.4/a/b`); (Q3) **every value property typed** by a defined value type (added domain types + enums to both catalogues); (Q4) **every MoP/MoE bound via a parametric diagram** (ReelCut 12/12 MoP constraint blocks, podcast 6/6 MoE constraint blocks). Per-UC activity diagrams were generated by a **draft→adversarial-review agent pipeline** (Opus; reviewer render-validated each Mermaid). Verified by an 8-agent **re-audit**; residual gaps (UC-6 activity, missing IBD connectors, port attribution) closed in a second iteration. **75 diagrams render (ReelCut 49 + podcast 26).** Report: `MBSE-COMPLETENESS-REPORT.md`. | User HARD rule + four explicit completeness questions; "brainstorm → plan → review plan → do → iterate → review", many agents with communication (pipeline stages). |

| ADR-016 | 2026-06-24 | Accepted | **Cross-session ("multi-chat") coordination protocol.** Multiple Claude Code sessions work this repo in parallel with no live channel; they coordinate **only via committed artifacts**, in precedence order: (1) `KNOWLEDGE.md` (shared facts, auto-loaded; the graphify/"cavemem" channel), (2) `DECISIONS.md` (ADRs/waivers/RAID, auto-loaded), (3) git commit messages, (4) **GitHub PRs + PR comments** (comment on a sibling session's PR to flag conflicts / hand off), (5) `drift-check.sh` (shared gate). To reach another session, write it in `KNOWLEDGE.md` or comment on its PR. Handoff + active-branch list maintained in `KNOWLEDGE.md` §"Coordination & cross-session handoff". | User asked to coordinate with other chats; there is no inter-session messaging tool, so the durable shared state IS the channel. Surfaced a live conflict: sibling PR #3/#4 edit `KNOWLEDGE.md` from a pre-PR-7 base and must rebase before merge. |
...
| I-9 | Open | Each `diagrams/render.sh` writes a **cwd-local `.pptr.json`** and removes it at the end, so running the SAME model's renderer twice **concurrently** makes the two runs clobber each other's puppeteer config → spurious "failed" diagrams. | Operational only (no model defect): run a given model's `render.sh` once at a time, or make the temp config name unique per process (`$$`). Surfaced 2026-06-24. |
| I-8 | Closed | **Podcast need IDs have an unused N-28** (sequence runs N-01…N-33 with N-28 absent). Found during the SysML-completeness audit. | No element anywhere references N-28 — it is an intentional numbering gap, not a dangling link. Documented in `13-parametrics-and-requirements.md` §13.4.4; no action needed. |
